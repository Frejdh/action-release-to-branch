...
### Example 3. Node project with deployment to GitHub pages
In the event that the build project shall be pushed to another repository, you'd first need to configure a GitHub action secret.
...
env:
  ...
  NODE_VERSION: 24
  RELEASE_REPOSITORY: 'variable is mapped later'
  RELEASE_BRANCH: pages_fallback
  ...
  workflow_dispatch:
    inputs:
      ...
      deployment-environment:
        description: The deployment environment to target
        type: choice
        default: Development
        options:
          - Development
      ...
      node-build-script:
        description: The script name defined in package.json for building the project
        default: build
        required: false

      node-build-target-directory:
        description: The directory where the built Node files are located
        default: dist/ui
        required: false

      delete-old-node-files:
        type: boolean
        default: true
        description: Whether to delete old node files (for GitHub pages)
        required: false

      node-files-to-keep:
        default: '^README.md$, ^readme.md$, ^[.].*'
        description: Whether to delete old node files. Only applicable 'delete-old-node-files' is enabled. Comma separated regex patterns
        required: false
      ...
      create-tag-pattern:
        description: The pattern that should generate tags when enabled. JavaScript regex syntax
        required: false
        default: '^(?!.+(-SNAPSHOT)$).+$'
  ...
  release:
    ...
    steps:
      - name: Resolve deployment environment
        shell: bash
        run: |
          if [[ "${{ inputs.deployment-environment }}" == "Production" ]]; then
            echo "Resolved production environment"
            echo "RELEASE_REPOSITORY=Frejdh/Frejdh-action-release-node-test-prod" >> $GITHUB_ENV
          elif [[ "${{ inputs.deployment-environment }}" == "Staging" ]]; then
            echo "Resolved staging environment"
            echo "RELEASE_REPOSITORY=Frejdh/Frejdh-action-release-node-test-stg" >> $GITHUB_ENV
          else
            echo "Resolved development environment"
            echo "RELEASE_REPOSITORY=Frejdh/Frejdh-action-release-node-test-dev" >> $GITHUB_ENV
          fi
      - name: Build node project and push to GitHub pages repository
        uses: Frejdh/action-release-to-branch@node-project-support
        with:
          github-token: ${{ secrets.RELEASE_PAT_TOKEN || secrets.GITHUB_TOKEN }}
          project-framework: ${{ env.PROJECT_FRAMEWORK }}
          build-arguments: ${{ inputs.build-arguments }}
          commitish: ${{ inputs.commitish || env.EVENT_BRANCH_NAME }}
          create-tag-enabled: ${{ inputs.create-tag-enabled || env.DEFAULT_TAG_ENABLED }}
          create-tag-pattern: ${{ inputs.create-tag-pattern || env.DEFAULT_TAG_PATTERN }}
          create-tag-allow-override: ${{ inputs.create-tag-allow-override || env.DEFAULT_TAG_OVERRIDE }}
          release-branch-repository: ${{ env.RELEASE_REPOSITORY }}
          release-branch-name: ${{ env.RELEASE_BRANCH }}
          node-build-script: ${{ inputs.node-build-script }}
          node-build-target-directory: ${{ inputs.node-build-target-directory }}
